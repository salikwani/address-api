...
function getDetails(e) {
    ...
    fetch(`https://api.geoapify.com/v1/geocode/search?text=${str}&apiKey=a9212495cb8b43b6ab6eb2d331dd1a11`).then(response => response.json())
    .then(result => {
        console.log(result);
        if (result.features.length === 0) {
            document.getElementById("address").innerHTML += "<p>Please enter a valid address</p>";
        } else {
            document.getElementById("address").innerHTML += `<h1>Your Result</h1>
                                                            <div>
